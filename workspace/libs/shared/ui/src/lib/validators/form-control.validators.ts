...
export class FormControlValidators {
  ...
  public static email(control: AbstractControl): ValidationErrors | null {
    ...
    return valid
      ? null
      : { email: true, ...FormControlValidators.trim(control) };
  }
  ...
  public static multipleEmails(
    ...
    const regExp =
      /^([a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,})(,(\s)?[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,})*$/i;
    const valid = control.valid && regExp.test(control.value);
    return valid
      ? null
      : { emails: true, ...FormControlValidators.trim(control) };
  }
  ...
  public static atLeast(
    ...
    const atLeast = control.value?.length && control.value.length >= minNumber;
