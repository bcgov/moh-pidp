...
import { collegeLicenceInfoResolver } from './college-licence-info.resolver';

describe('collegeLicenceInfoResolver', () => {
  ...
  describe('METHOD: ResolveFn', () => {
    ...
    given('partyId exists, user has no CPN (College certification)', (done) => {
      const partyId = randNumber({ min: 1 });
      partyServiceSpy.accessorSpies.getters.partyId.mockReturnValue(partyId);
