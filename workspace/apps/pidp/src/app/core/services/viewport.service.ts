...
import { DeviceResolution } from '../enums/device-resolution.enum';

import { WindowRefService } from './window-ref.service';

// TODO can this be deprecated for CDK Observers?
...
export class ViewportService {
  ...
  public onResize(): Observable<DeviceResolution> {
    ...
    return this.viewport.pipe(distinctUntilChanged());
  }
  ...
  private setupViewportListener(): Observable<DeviceResolution> {
    return new Observable((observer) => {
      this.window.addEventListener('resize', (_: UIEvent) =>
