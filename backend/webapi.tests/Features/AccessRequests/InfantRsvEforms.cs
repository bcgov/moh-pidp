...
public class InfantRsvEformsTests : InMemoryDbTest
{
    [Theory]
    [MemberData(nameof(InfantRsvEformsIdentifierTypeTestData))]
    public async Task CreateInfantRsvEformsEnrolment_ValidProfileWithVaryingLicence_MatchesAllowedTypes(IdentifierType identifierType, bool expected)
    {
        ...
        var handler = this.MockDependenciesFor<InfantRsvEforms.CommandHandler>(client, keycloak);

        var result = await handler.HandleAsync(new InfantRsvEforms.Command { PartyId = party.Id });
        ...
        if (expected)
        {
            ...
            {
                A.CallTo(() => keycloak.AssignAccessRoles(credential.UserId, MohKeycloakEnrolment.InfantRsvEforms)).MustHaveHappened();
            }
            foreach (var credential in party.Credentials.Where(credential => credential.IdentityProvider != IdentityProviders.BCServicesCard))
            {
                A.CallTo(() => keycloak.AssignAccessRoles(credential.UserId, MohKeycloakEnrolment.InfantRsvEforms)).MustNotHaveHappened();
            }
            Assert.Contains(this.TestDb.AccessRequests, request => request.PartyId == party.Id
                && request.AccessTypeCode == AccessTypeCode.InfantRsvEforms);
        }
        else
        {
    ...
    public static TheoryData<IdentifierType, bool> InfantRsvEformsIdentifierTypeTestData()
    {
        var testData = new TheoryData<IdentifierType, bool>();

        foreach (var identifierType in TestData.AllIdentifierTypes)
        {
            testData.Add(identifierType, InfantRsvEforms.AllowedIdentifierTypes.Contains(identifierType));
        }
    ...
    [Fact]
    public async Task CreateInfantRsvEformsEnrolment_CpsPostgradResident_Success()
    {
        ...
        A.CallTo(() => keycloak.AssignAccessRoles(A<Guid>._, MohKeycloakEnrolment.InfantRsvEforms)).MustHaveHappened();
    }

    [Fact]
    public async Task CreateInfantRsvEformsEnrolment_NursePractitioner_Success()
    {
    ...
    public async Task CreateInfantRsvEformsEnrolment_NoCpn_UsesEndorsementStanding(PlrStandingsDigest endorsementDigest, bool expected)
    {
    ...
    [Fact]
    public async Task CreateInfantRsvEformsEnrolment_AlreadyEnroled_Denied()
    {
        var party = this.TestDb.HasAParty(party =>
        {
            party.Email = "Email@email.com";
            party.Cpn = "Cpn";
            party.Credentials = [new Credential { UserId = Guid.NewGuid(), IdentityProvider = IdentityProviders.BCServicesCard }];
            party.AccessRequests = [new AccessRequest { AccessTypeCode = AccessTypeCode.InfantRsvEforms, RequestedOn = Instant.FromUtc(2026, 1, 1, 0, 0) }];
        });
        var client = A.Fake<IPlrClient>()
    ...
    [Fact]
    public async Task CreateInfantRsvEformsEnrolment_NoBcscCredential_Denied()
    {
    ...
    [Fact]
    public async Task CreateInfantRsvEformsEnrolment_NoEmail_Denied()
    {
        var party = this.TestDb.HasAParty(party =>
        {
            party.Email = null;
