...
public class InfantRsvEformsRevocationTests : InMemoryDbTest
{
    [Fact]
    ...
    public async Task RevokeIfIneligible_NoLongerEligible_RemovesRoleAndAccessRequest()
    {
        ...
        A.CallTo(() => keycloak.RemoveAccessRoles(party.Credentials.Single().UserId, MohKeycloakEnrolment.InfantRsvEforms)).MustHaveHappened();
    ...
    public async Task RevokeIfIneligible_KeycloakFails_LeavesAccessRequestToBeRetried()
    {
        ...
        var service = this.MockDependenciesFor<InfantRsvEformsRevocationPolicy>(plr, keycloak, revocationService);
    ...
    public async Task RevokeIfIneligible_VaryingLicence_RevokesWhenNotAllowed(IdentifierType identifierType, bool stillEligible)
    {
        ...
        {
            A.CallTo(() => keycloak.RemoveAccessRoles(A<Guid>._, MohKeycloakEnrolment.InfantRsvEforms)).MustHaveHappened();
            Assert.DoesNotContain(this.TestDb.AccessRequests, request => request.PartyId == party.Id);
        }
    }
    ...
    /// </summary>
    private (IPlrClient Plr, IKeycloakAdministrationClient Keycloak, InfantRsvEformsRevocationPolicy Policy) SetupFor(PlrStandingsDigest digest)
    {
        ...
        return (plr, keycloak, this.MockDependenciesFor<InfantRsvEformsRevocationPolicy>(plr, keycloak, revocationService));
    }
}
